...
export const Skeleton = memo(function Skeleton({
    ...
    return (
        <motion.div
            className={`${height} ${width} ${className} rounded-lg bg-gradient-to-r from-zinc-800 via-zinc-700 to-zinc-800 bg-[length:200%_100%]`}
...
export const StatsSkeleton = memo(function StatsSkeleton() {
    return (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {[...Array(4)].map((_, i) => (
                <div key={i} className="bg-zinc-900/50 rounded-2xl p-6 border border-white/10">
...
export const QuickActionsSkeleton = memo(function QuickActionsSkeleton() {
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[...Array(4)].map((_, i) => (
                <div key={i} className="bg-zinc-900/50 rounded-2xl p-5 border border-white/10 text-center">
                ...
                {[...Array(12)].map((_, i) => {
                    ...
                    return (
                        <div
                            ...
                            className="w-full rounded-t-lg bg-zinc-700 animate-pulse opacity-60"
                        />
                    );
                })}
            </div>
